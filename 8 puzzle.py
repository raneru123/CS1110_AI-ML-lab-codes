...
class Node:
    def __init__(self, current_node, previous_node, m, h, dir):
...
def getAdjNode(node):
    ...
    for dir in DIRECTIONS.keys():
        ...
        if 0 <= newPos[0] < len(node.current_node) and 0 <= newPos[1] < len(node.current_node[0]):
            ...
            # listNode += [Node(newState, node.current_node, node.m + 1, euclidianCost(newState), dir)]
            listNode.append(Node(newState, node.current_node, node.m + 1, euclidianCost(newState), dir))
